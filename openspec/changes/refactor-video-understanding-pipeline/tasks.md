...
- [ ] 2.1 提取时间规范化、reasoning 分离、事件与证据构造 helper，并通过无 I/O 单元测试。
- [ ] 2.2 提取模型响应到 `UnderstandingResult` 的单一转换边界，保持所有默认字段和兼容行为。
- [ ] 2.3 保留 `video_understanding.py` 的公共 import、工具注册和 monkeypatch 兼容 facade。

## 3. 拆分 I/O 编排

- [ ] 3.1 将帧获取/编码和 VLM 调用整理为显式可注入边界。
- [ ] 3.2 将短视频、长视频和 RTSP/VST source adapter 从结果转换中分离。
- [ ] 3.3 迁移 LVS 路径复用语义一致的 helper，并显式保留 sensor、source type 和时间差异。
