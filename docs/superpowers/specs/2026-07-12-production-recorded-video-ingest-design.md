...
启动器继续以 `scripts/es-runtime-stack.sh` 和对应 PowerShell 入口作为唯一用户入口，并将 `--data-root` 解析为运行时根目录。每次启动生成 UUID `run_id`，日志和进程清单写入 `.runtime/es-stack/runs/{run_id}/`，`latest` 仅指向最近一次运行。启动器必须显式启动并等待 API、Worker、UI 和 ES readiness，记录每个子进程的 PID、命令、启动时间和退出状态；停止或回收端口时只处理当前用户拥有的进程，不使用 sudo。
