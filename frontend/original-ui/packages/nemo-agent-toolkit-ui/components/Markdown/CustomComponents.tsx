  ...
  return {
    code: memo(
      ({
        ...
        const rawChildString = String(children);
        const childString = rawChildString.replace(/\n$/, '');
        const isInlineCode =
          inline === true || (!match && !rawChildString.endsWith('\n'));
    ...
    video: memo(
      ...
      (prevProps, nextProps) => {
        ...
        if (prevProps.src?.length > 1000 || nextProps.src?.length > 1000) {
          ...
          const prevStart = prevProps.src?.substring(0, 100) || '';
          const prevEnd =
            prevProps.src?.substring(prevProps.src.length - 100) || '';
          const nextStart = nextProps.src?.substring(0, 100) || '';
          const nextEnd =
            nextProps.src?.substring(nextProps.src.length - 100) || '';
